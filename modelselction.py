...
def supermeModelselction(xfile,modelSavefile,filename,suptxt):
    ...
    for output in sheetnames:
        ...
        if maxCol == 1:
            ...
            print(supremeModel)
            #input("fr")
            modelSavefile.cell(row=2,column=1+outputcol).value=roFreqModel
            modelSavefile.cell(row=3, column=1 + outputcol).value = freqratio
            modelSavefile.cell(row=4, column=1 + outputcol).value = rmseDict[roFreqModel]
            ...
            supinfo = filename+"+"+output+":"+supremeModel
...
for xlfile in file_list:
    ...
    suptxt.close()
summarywb.save('dataset_new_avg/result/modelselect/supremeselectModel.xlsx')
print("FINISH!")
