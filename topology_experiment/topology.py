...
if __name__ == '__main__':
    ...
    prompt = "connection per tenant?"
    cnum = input(prompt)
    cnum = int(cnum)

    # dn = raw_input("Virtual network switch? ")
    # dn = int(dn)
    ...
    # s1 = hosts[2]
    # c1 = hosts[endhost]
    # print(s1.name,s1.params['ip'],c1.name,c1.params['ip'])

    # result1 = s1.cmd("iperf -s -1 -p 5301 >> logResult/iperfResult/iperf_%s_s.txt &" % (current_time))
    ...
    c2 = hosts[3]
    s2 = hosts[15]
    print("Generate Traffic bewteen %s and %s using port(%s)" % (c2.params['ip'], s2.params['ip'], ppport))
    result3 = s2.cmd("iperf -s -1 -p 6000 > logResult/iperfResult/v2_host_s.txt &")
    result4 = c2.cmd("iperf -c 192.0.0.15 -p 6000 -n 1 -l 100 -P 1 > logResult/iperfResult/v2_host_c.txt &")
